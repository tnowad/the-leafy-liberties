<div class="flex flex-row justify-around w-3/4 md:flex-col md:justify-start md:w-1/4 p-4">
  <div class="flex flex-col items-center justify-center">
    ...
    <div onclick="document.querySelector('#upload-image').click()" class="absolute text-4xl cursor-pointer">
      <!-- upload -->
      <i class="fa-regular fa-camera rounded-full p-16 hover:bg-gray-200 hover:backdrop-blur-xl transition-all opacity-0 hover:opacity-100 "></i>
    </div>
  </div>
  <div class="box-border flex flex-col items-start justify-center px-6">
    ...
    <a href="<?php echo BASE_URI .
                "/profile/purchase_history"; ?>" class="w-40 flex mt-5 p-2 hover:bg-primary-600 hover:text-white rounded-lg transition-all">
      <!-- <i class="flex items-center fa fa-check mr-2"></i> -->
      <i class="fas fa-history flex items-center mr-2"></i>
...
<script>
  ...
  for (var i = 0; i < links.length; i++) {
    if (links[i].getAttribute("href") === currentPath) {
      links[i].classList.add("bg-primary-600", "text-white");
      console.log(links[i].classList)
    } else {
      links[i].classList.remove("bg-primary-600", "text-white");
    }
  }
</script>