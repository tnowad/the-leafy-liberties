    ...
    <div className="table-product-statistics my-8 shadow-lg cursor-pointer rounded-2xl overflow-hidden bg-white">
      <div className="relative">
        ...
        <table className="w-full text-sm text-center text-gray-500 rounded-2xl border-collapse overflow-hidden">
          <thead className="text-xs text-gray-700 uppercase bg-gray-50">
            <tr>
              {arrayTitle.map((item) => {
              return (
              <th key={item.name} scope="col" colSpan={item.action==='Action' ? 2 : 1} className="px-6 py-3">
                {item.name}
              </th>
              )
              })}
          ...
          <tbody>
            ...
            return (
            <tr key={item.id} className="bg-white border-b hover:bg-gray-200 transition-opacity">
        ...
        <>
          {productData.map((item) => {
          return (
          <table key={item.id} className="w-full text-sm text-left text-gray-500 border-collapse overflow-hidden flex flex-row justify-between border-0 border-solid border-gray-200 border-b-[1px]">
    ...
    <div className="absolute top-0 left-0 h-full w-full flex justify-center items-center bg-gray-500 bg-opacity-75 z-10">
      <div className="bg-white p-8 rounded-md shadow-lg w-[500px]">
        ...
        <form className="flex flex-col" onSubmit={handleSubmit}>
          ...
          <button className="mt-2 bg-green-800 hover:bg-green-600 transition-colors text-white font-bold py-2 px-4 rounded" type="submit">
            Submit
          </button>
          <button className="mt-2 bg-gray-500 hover:bg-gray-700 text-white font-bold py-2 px-4 rounded " onClick={()=> setIsFormVisible(false)}
            >
            Cancel
          </button>
        </form>
