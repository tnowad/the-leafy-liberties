<div class="w-full my-0 mx-auto overflow-x-hidden">
  <div class="mt-10 min-h-screen box-border w-full px-10 mobile:px-5">
    <div class="flex justify-between">
      ...
      <button class="w-5 h-5 text-2xl ">
        +
      </button>
    </div>
    <div class="table-customer-statistics my-8 shadow-lg cursor-pointer rounded-2xl overflow-hidden bg-white">
      <div class="relative">
        ...
        <table class="w-full text-sm text-center text-gray-500 rounded-2xl border-collapse overflow-hidden">
          <thead class="text-xs text-gray-700 uppercase bg-gray-50">
            <tr>
              {arrayTitle.map((item) => {
              return (
              <th scope="col" colSpan={item.action==='Action' ? 2 : 1} class="px-6 py-3">
                {item.name}
              </th>
              )
              })}
        ...
        <>
          {customerData.map((item) => {
          return (
          <table class="w-full text-sm text-left text-gray-500 border-collapse overflow-hidden flex flex-row justify-between border-0 border-solid border-gray-200 border-b-[1px]">
    ...
    <div class="absolute top-0 left-0 h-full w-full flex justify-center items-center bg-gray-500 bg-opacity-75 z-10">
      <div class="bg-white p-8 rounded-md shadow-lg w-[500px]">
        ...
        <form class="flex flex-col" onSubmit={handleSubmit}>
          ...
          <input type="number" value={totalBuy} onChange={(e)=> setTotalBuy(e.target.value)}
          class="bg-gray-100 px-2 focus:outline-none rounded-lg"
          />
