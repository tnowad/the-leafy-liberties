<div>
  <div className="my-10 md:my-16
    ...
    <FontAwesomeIcon className="text-4xl md:text-6xl text-green-800" icon={faHeart} />
  ...
  <table className="w-full border-collapse">
    <thead className="w-full bg-gray-100 rounded-sm">
      <tr className="w-44">
        {arrayTitle.map((item) => {
        return (
        <th colSpan={item.name==='Action' ? 2 : 1} className="px-4 py-2" key={item.name}>
          {item.name}
        </th>
        )
        })}
  ...
  <>
    {productData.map((item) => {
    return (
    <table key={item.id} className="ml-5 mb-8 pt-10 flex flex-row justify-around border-0 border-t-2 border-gray-200 border-solid ">
      <thead>
        <tr className="flex flex-col">
          {arrayTitle.map((item) => {
          return (
          <th className={ item.name==='Action' ? `h-16 pt-5` : `h-auto` } key={item.name}>
            {item.name}
          </th>
          )
          })}
